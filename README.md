...
## 🚀 Quick start / Cómo reproducir los resultados
1. Instala dependencias con `pip install -r requirements.txt` (o `conda env create -f environment.yml`).
2. Ejecuta el pipeline completo:
   ```bash
   python -m src.run_all
   ```
   Todos los artefactos (modelos entrenados, métricas, intervalos bootstrap y gráficos) se guardan en `results/`.

### 📏 Evaluation protocol
\paragraph{Evaluation protocol.}
Final performance was assessed on the held-out test set using the coefficient of determination ($R^2$) and Mean Absolute Error (MAE). We report point estimates in the main text; 95 % confidence intervals via bootstrap (1{,}000 resamples) and additional diagnostics (residual plots vs. predictions and vs. key features, learning curves, and $y$-scrambling tests) are provided in the Supplementary Information (SI). Physical plausibility was checked post hoc against known constraints on descriptors and trends from the literature.
...
- Modelos entrenados: `ridge_model.joblib`, `lasso_model.joblib`, `rf_model.joblib`.
- Métricas de prueba y coeficientes: `metrics.json`, `ridge_coefficients.csv`, `rf_feature_importances.csv`.
- Intervalos de confianza (bootstrap): `bootstrap_intervals.json` y ficheros `bootstrap_*.csv`.
- Diagnósticos y gráficas: `parity_*.png`, `residuals_*.png` y `diagnostic_metrics.csv`.
- Example plot auto-generated by `python -m src.run_all`:

> Nota: los artefactos generados (p. ej., gráficas PNG) no se versionan en el repositorio. Ejecute `python -m src.run_all` para regenerarlos en `results/` cuando sea necesario.
